...
import com.swisstph.expcreator.arms.ArmXml;
...
public class SweepXml extends Sweep {
    ...
    public SweepXml(String fName, File[] armXmls) throws Exception {
        ...
        if (refArm == -1) {
            System.out.println("Warning: no reference specified — using first arm as reference.");
    ...
    public void updateDb(Connection conn, int sweepIndex) throws Exception {
        ...
        pstmt.setInt(4, sweepIndex);	// always use a unique sweep group (fully factorial design)
        pstmt.executeUpdate();
        ...
        stmt.execute(
                "UPDATE sweeps SET ref_arm_id=" + Integer.toString(arms.get(refArm).getId()) + " WHERE swe_id=" + Integer.toString(id));
