...
import com.swisstph.expcreator.patchtree.PTElement;
import com.swisstph.expcreator.patchtree.PTNodeList;
import com.swisstph.expcreator.patchtree.PTReplaceAttr;
import com.swisstph.expcreator.patchtree.PTReplaceNode;
import com.swisstph.expcreator.patchtree.PTReplaceNodeList;
...
public class ArmXml extends Arm {
    ...
    public ArmXml(Element base, File xml) throws Exception {
    	...
    	super(base,xml);
    	assert xml.getName().endsWith(".xml");	// should already have been checked
    ...
    public ArmXml(int n) {
        ...
        patch = new PTElement("scenario",
            new PTNodeList("model", new PTElement("model",
                new PTNodeList("parameters", new PTElement("parameters",
                    new PTReplaceAttr(seed)
                ) )
            ) )
        );
    }
    ...
    public Document apply(Document document) throws Exception {
        ...
        try {
            ...
            if (patch != null) {
                /* n = */ patch.apply(document, null, document.getDocumentElement());
            }
        } catch (Exception e) {
            throw e;
        }
    ...
    private PTBase diffRecurse(Node armNode, Node baseNode) throws Exception {
        ...
        switch (nodeType) {
            ...
            case Node.TEXT_NODE:
                if (!armNode.getTextContent().equals(baseNode.getTextContent())) {
                    assert armNode.getNodeName().equals("#text") || armNode.getNodeName().equals("#comment");
                    // Text content is different; is treated as a Node in the DOM tree
        ...
        for (int i = 0; i < len; ++i) {
            ...
            if (!armAttr.getNodeName().equals(baseAttr.getNodeName())) // different attributes: this element differs
            {
                return new PTReplaceNode(armNode);
            }
        ...
        // If non-empty, contents are aggregated along with differing sub-elements.
        ...
        /* We now have a list of all sub-element lists.
         * For each sub-element list: */
        Iterator<Entry<String, ArmBasePair>> it = eltLists.entrySet().iterator();
        while (it.hasNext()) {
            ...
            if (ab.arm.size() == ab.base.size()) {
                ...
                if (allEqual) // all elements equal so nothing to patch
                {
                    ...
                    differentNodes.add(
                            new PTNodeList(entry.getKey(), patchList));
                }
            } else {
                ...
                differentNodes.add(
                        new PTReplaceNodeList(entry.getKey(), nextElement, ab.arm));
            }
        }
