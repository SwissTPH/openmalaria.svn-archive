...
public class ScenarioArmAssociation {
	...
	public Document applyArms(Document wu,boolean uniqueSeeds,PrintStream scnListOut, ArrayList<Sweep> sweeps) throws Exception {
		CharArrayWriter wName = new CharArrayWriter(128);
		wName.append(getFileName());
		scnListOut.print(getFileName());
		for (int i = 0; i < armIndecies.length; ++i) {
			sweeps.get(i).writeNamePair(wName, armIndecies[i], scnListOut);
		}
		scnListOut.println();
